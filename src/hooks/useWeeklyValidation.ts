...
export function useWeeklyValidation(enabled: boolean, date?: string, weekNumber?: number) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await meService.getWeeklyValidation(date, weekNumber);
      setData(response);
    } catch (e: any) {
